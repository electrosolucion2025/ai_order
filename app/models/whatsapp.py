...
def delete_old_records(session, *args, **kwargs):
    expiration_date = datetime.now() - timedelta(days=1)  # Adjust the timedelta as needed
    session.query(ProcessedMessage).filter(ProcessedMessage.created_at < expiration_date).delete()
    session.commit()
